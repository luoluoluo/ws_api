...
func (t *Task) Tasks(lastID int, size int) (*TaskPaginator, error) {
	var tasks = []Task{}
	var hasMore = false
	var count int
	taskCountSQL := "SELECT COUNT(*) FROM task where status=1"
	if lastID != 0 {
		taskCountSQL += " AND id < " + strconv.Itoa(lastID)
	}
	...
	taskSQL := "SELECT t.*, u.name AS user_name, u.avatar AS user_avatar FROM task t LEFT JOIN user u ON t.user_id=u.id WHERE t.status=1"
	if lastID != 0 {
		taskSQL += " AND t.id < " + strconv.Itoa(lastID)
	}
	taskSQL += " ORDER BY id DESC LIMIT " + strconv.Itoa(size)
